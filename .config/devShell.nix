        ...
        getChangedAtSha = pkgs.writeShellApplication {
          ...
          text = ''
            ...
            if [ -z "$SHA" ]; then
              ...
              done < <(git diff-tree -r --name-only "$SHA^..$SHA" -- .)
            fi
        ...
        // (
          ...
          then {
            ...
            lint = pkgs.writeShellApplication {
              ...
              runtimeInputs = [pkgs.git devShellConfig.packages.lint getChangedAtSha];
              text = ''
                SHA="''${1:-}"
                ...
                if [ -n "$FILES" ]; then
                  lint "$FILES" || (echo "failed to lint $(realpath .)" >&2 && exit 1)
                else
        ...
        // (
          ...
          then {
            ...
            build = pkgs.writeShellApplication {
              ...
              runtimeInputs = [pkgs.git devShellConfig.packages.build getChangedAtSha];
              text = ''
                SHA="''${1:-}"
                ...
                if [ -n "$FILES" ]; then
                  build "$FILES" || (echo "failed to build $(realpath .)" >&2 && exit 1)
                else
        ...
        // (
          ...
          then {
            ...
            runTest = pkgs.writeShellApplication {
              ...
              runtimeInputs = [pkgs.git devShellConfig.packages.runTest getChangedAtSha];
              text = ''
                SHA="''${1:-}"
                ...
                if [ -n "$FILES" ]; then
                  runTest "$FILES" || (echo "failed to test $(realpath .)" >&2 && exit 1)
                else
